...
import uri from "url:../parse-data-rs/data.torrent";
...
export class TorrentDataProvider {
    ...
    constructor(private progress: (message: string) => void) {
        ...
        this.torrent.on("infoHash", () =>
            console.log("infoHash", this.torrent.infoHash)
        );
        ...
        this.torrent.on("warning", (e) => {
            if (
                e &&
                e instanceof Error &&
                e.message.startsWith("Unsupported tracker protocol")
            )
                return;
            console.log(e);
        });
    ...
    private async _indexReady() {
        ...
        const fmap = new Map(
            this.torrent.files.map((f) => [f.name, f] as [string, TorrentFile])
        );
    ...
    async getNames() {
        ...
        const blobUrl = await promisify(
            inx.getBlobURL.bind(inx) as typeof inx.getBlobURL
        )();
    ...
    async getSeriesInfo(seriesName: string) {
        ...
        if (!this.cFileMap.get(fname)) {
            ...
            const getBuffer = promisify(
                f.getBuffer.bind(f) as typeof f.getBuffer
            );
