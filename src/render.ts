async function getArrayBuffer(filename: string) {
    const ret = await fetch(filename);
    if (!ret.ok) throw ret.status;
...
function seriesToSeasons(series: Series) {
    ...
    for (const ep of series.episodes) {
        ...
        const season =
            seasons[ep.season] || (seasons[ep.season] = { min: Infinity, max: -Infinity });
...
function showChart(series: IndexedSeries[]) {
    ...
    const chartSeries = series.map(([i, s], si) => ({
        name: s.title,
        type: ({ undefined: "scatter", bar: "column", scatter: "scatter", line: "line" } as any)[
            "" + qd["d"]
        ],
        marker: {
            enabled: true,
...
async function load() {
    ...
    const seriesList = await (await fetch("chunks.json")).json();
    ...
    autoComplete = new AutoComplete("search", {
        ...
        lists: {
            series: {
                ...
                //tokenHTML: x => "a"
                matchOptions: (inp: string, res: any[]) =>
                    res.slice(0, 20).filter(v => !currentDisplay.some(([i, s]) => i === v.value))
            }
        },
        onChange: (val: any[]) =>
            showChart(val.map(v => [v[0].value, database.series[v[0].value]] as IndexedSeries))
    });
